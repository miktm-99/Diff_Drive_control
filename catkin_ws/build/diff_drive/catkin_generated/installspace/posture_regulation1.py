...
class PointNavigationController:
    def __init__(self):
        ...
        self.k3 = rospy.get_param('~k3', 3.0)
        ...
        # Generate trajectory with 10 points
        self.trajectory = np.linspace([self.x, self.y], [self.x_goal, self.y_goal], num=10)
    ...
    def posture_regulation_control(self, x_target, y_target):
    ...
    def control_loop(self, event):
        if self.current_target_index < len(self.trajectory):
            x_target, y_target = self.trajectory[self.current_target_index]
            v, omega, rho = self.posture_regulation_control(x_target, y_target)
            
            # Move to the next point if close to current target
            if rho < 0.2:
                self.current_target_index += 1
                rospy.loginfo(f"Reached waypoint {self.current_target_index}/{len(self.trajectory)}")
            ...
            # Stop the robot at the final destination
    ...
    def plot_trajectory(self):
        ...
        plt.plot(self.trajectory[:, 0], self.trajectory[:, 1], 'go', label='Waypoints')
